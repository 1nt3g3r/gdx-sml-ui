 ...
 *     Name                 Description
 *     -------------------- ----------------------------------------------------------------
 *     E                    The float value that is closer than any other to e, the base of the natural logarithms (2.718281828459045).
 ...
 *     PI                   The float value that is closer than any other to pi, the ratio of the circumference of a circle to its diameter (3.141592653589793).
 * </pre>
 * <p>
...
public class MathEval extends Object {
	...
	private final SortedMap<String, Float> constants; // external constants
	private final SortedMap<String, Float> variables; // external variables
	...
	public MathEval() {
		...
		setConstant("E", (float) Math.E);
		setConstant("Euler", 0.577215664901533f);
		setConstant("LN2", 0.693147180559945f);
		setConstant("LN10", 2.302585092994046f);
		setConstant("LOG2E", 1.442695040888963f);
		setConstant("LOG10E", 0.434294481903252f);
		setConstant("PHI", 1.618033988749895f);
		setConstant("PI", (float) Math.PI);
	 ...
	 */
	public float getConstant(String nam) {
		Float val = constants.get(nam);
		return (val == null ? 0 : val.floatValue());
	}

	/** Gets an unmodifiable iterable of the constants in this evaluator. */
	public Iterable<Map.Entry<String, Float>> getConstants() {
	 ...
	 * name have precedence over constants.
	 */
	public MathEval setConstant(String nam, Float val) {
		return setConstant(nam, Float.valueOf(val));
	}
	 ...
	 */
	private float _evaluate(int beg, int end) throws NumberFormatException, ArithmeticException {
		return _evaluate(beg, end, 0.0f, OPERAND, getOperator('='));
	}
	 ...
	 */
	private float _evaluate(int beg, int end, float lft, Operator pnd, Operator cur)
		...
		for (ofs = beg; (ofs = skipWhitespace(expression, ofs, end)) <= end; ofs++) {
			float rgt = Float.NaN; // next operand (right-value) to process
...

			{
				...
				if (beg == ofs && (cur.unary == LEFT_SIDE || nxt.unary == RIGHT_SIDE)) {
					rgt = Float.NaN; // left-binding unary operator; right value
										// will not be used and should be blank
				} else if (ch0 == '(') {
					...
					try {
						if (stringOfsEq(expression, beg, "0x")) {
							rgt = (float) Long.parseLong(expression.substring(beg + 2, ofs).trim(), 16);
						} else {
							rgt = Float.parseFloat(expression.substring(beg, ofs).trim());
						}
		...
		if (ofs > end && cur != OPERAND) {
			if (cur.unary == LEFT_SIDE) {
				lft = doOperation(beg, lft, cur, Float.NaN);
			} else {
	...
	static public interface OperatorHandler {
		public float evaluateOperator(float lft, char opr, float rgt) throws ArithmeticException;
	}
	...
	static public interface FunctionHandler {
		public float evaluateFunction(String fncnam, ArgParser fncargs) throws ArithmeticException;
	}
	...
	static class DefaultImpl extends Object implements OperatorHandler, FunctionHandler {
		...
		public float evaluateOperator(float lft, char opr, float rgt) {
			...
			case '^':
				return (float) Math.pow(lft, rgt); // power
			case '±':
		...
		public float evaluateFunction(String fncnam, ArgParser fncargs) throws ArithmeticException {
			...
			case 'a': {
				if (fncnam.equalsIgnoreCase("abs")) {
					return (float) Math.abs(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("acos")) {
					return (float) Math.acos(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("asin")) {
					return (float) Math.asin(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("atan")) {
					return (float) Math.atan(fncargs.next());
				}
			}
				break;
			case 'c': {
				if (fncnam.equalsIgnoreCase("cbrt")) {
					return (float) Math.cbrt(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("ceil")) {
					return (float) Math.ceil(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("cos")) {
					return MathUtils.cos(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("cosh")) {
					return (float) Math.cosh(fncargs.next());
				}
			}
				break;
			case 'e': {
				if (fncnam.equalsIgnoreCase("exp")) {
					return (float) Math.exp(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("expm1")) {
					return (float) Math.expm1(fncargs.next());
				}
			}
				break;
			case 'f': {
				if (fncnam.equalsIgnoreCase("floor")) {
					return (float) Math.floor(fncargs.next());
				}
			}
				break;
			case 'g': {
			...
			case 'l': {
				if (fncnam.equalsIgnoreCase("log")) {
					return (float) Math.log(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("log10")) {
					return (float) Math.log10(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("log1p")) {
					return (float) Math.log1p(fncargs.next());
				}
			}
				break;
			case 'm': {
			...
			case 'r': {
				...
				if (fncnam.equalsIgnoreCase("round")) {
					return (float) Math.round(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("roundHE")) {
					return (float) Math.rint(fncargs.next());
				} // round half-even
			}
				break;
			case 's': {
				if (fncnam.equalsIgnoreCase("signum")) {
					return (float) Math.signum(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("sin")) {
					return MathUtils.sinDeg(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("sinh")) {
					return (float) Math.sinh(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("sqrt")) {
					return (float) Math.sqrt(fncargs.next());
				}
			}
				break;
			case 't': {
				if (fncnam.equalsIgnoreCase("tan")) {
					return (float) Math.tan(fncargs.next());
				}
				if (fncnam.equalsIgnoreCase("tanh")) {
					return (float) Math.tanh(fncargs.next());
				}
			...
			case 'u': {
				if (fncnam.equalsIgnoreCase("ulp")) {
					return (float) Math.ulp(fncargs.next());
				}
			}
				break;
			// no default
